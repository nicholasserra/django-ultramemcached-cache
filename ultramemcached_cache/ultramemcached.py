...
class BaseUltraMemcachedCache(BaseCache):
    ...
    @_make_conn
    def add(self, client, key, val, timeout=0, version=None):
        key = self.make_key(key, version=version)
        val, flags = self._val_to_store(val)
    ...
    @_make_conn
    def set(self, client, key, val, timeout=0, version=None):
        key = self.make_key(key, version=version)
        val, flags = self._val_to_store(val)
    ...
    @_make_conn
    def set_many(self, client, data, timeout=0, version=None):
        ...
        for key, val in safe_data.items():
            ...
            client.set(key, val, self._get_memcache_timeout(timeout), flags)
